...
int main(int argc, char * argv[]) {
    ...
    char * stack = (char * ) malloc(STACK_SIZE * 3);
    if (!stack) {
    ...
    if (clone(client -> manRequest, stack + STACK_SIZE, CLONE_VM, & MiddlewareBB::Global::cache_man) == -1) {
        perror("Clone manRequest fail");
        exit(1);
    }
    if (clone(cache -> removeExpiredFiles, stack + STACK_SIZE + STACK_SIZE, CLONE_VM, & MiddlewareBB::Global::cache_man) == -1) {
        perror("Clone removeExpiredFiles fail");
        exit(1);
    }
    if (clone(server -> listener, stack + STACK_SIZE + STACK_SIZE + STACK_SIZE, CLONE_VM, & MiddlewareBB::Global::cache_man) == -1) {
