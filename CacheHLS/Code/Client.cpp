...
MiddlewareBB::Client::Client(unsigned long targetip, int port) {
    ...
    //First time add all chunk in manifest
    for (int i = 2; i < f.length; i++) {
        if ((f.data[i - 2] == '.') && (f.data[i - 1] == 't') && (f.data[i] == 's')) {
            for (int a = i; a > 1; a--) {
                if ((f.data[a - 1] == ' ') || (f.data[a - 1] == '\n')) {
                    strcpy(ts, manifest_path); //add manifest path
                    memcpy(ts + strlen(manifest_path), & f.data[a], i - a + 1); //path + ts name
                    memcpy(last_chunk, ts, strlen(ts)); //update last chunk

                    //Push on ts_list
                    while (!MiddlewareBB::Global::lock_ts_list.try_lock());
                    MiddlewareBB::Global::ts_list.push(ts);
                    MiddlewareBB::Global::lock_ts_list.unlock();
                    break;
                }
            }
        }
...
//Thread that manages the chunk request
int MiddlewareBB::Client::tsRequest(void * list) {
    try {
        std::queue < std::string > * ts_list = (std::queue < std::string > * ) list;
        MiddlewareBB::Global::file f; //tmp file

        //Files .ts
        while (recv(MiddlewareBB::Global::client_sock, NULL, 1, MSG_PEEK | MSG_DONTWAIT) != 0 && MiddlewareBB::Global::running) {
            //If no ts in the list wait
            if (ts_list -> size() == 0) {
                usleep(0.5 * 1000000);
                continue;
            }

            //If there are ts take first and request
            f = getFile(strdup(ts_list -> front().c_str()));
                        ...
                        if ((f.data[a - 1] == ' ') || (f.data[a - 1] == '\n')) {
                            ...
                            if (strcmp(ts, last_chunk) != 0) {
                                ...
                                if (MiddlewareBB::Global::reference != nullptr) {
                                    while (!MiddlewareBB::Global::lock_cache_man.try_lock());
                                    for (auto it = MiddlewareBB::Global::cache_man.cbegin(); it != MiddlewareBB::Global::cache_man.cend();) {
